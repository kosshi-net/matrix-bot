...
async function main() {
	...
	*/                                                       
	...
	new Command("powerlevel [#rooms..] <@users..> [number]", async function (this:Bot, ctx:CommandContext) 
	{
	...
	.set_description(`Whitelists users (sets .onjoin database property to "whitelist")`)
	.set_level(50)
	.allow_any_room()
	.register(bot.cmd);
	...
	*/
	...
	.set_description("(DEBUG ONLY) Print user's database entry. If field specified, prints only that field.")
	.set_level(90)
	.allow_any_room()
	.register(bot.cmd);

	new Command("test [#rooms..] [@users..]", async function (this:Bot, ctx:CommandContext) {
		let context = {
			target: ctx.target,
			argv: ctx.argv,
			event:ctx.event,
