...
async function main() {
	...
	new Command("parse", async function (ctx: any) {
		let event = ctx.event;
		...
		if (event.content.format == "org.matrix.custom.html") {
			...
			for (let node of Array.from(root.childNodes)) {
				if (node.rawTagName == "a") {
					...
					if (id[0] == "#") ret.target.room_id.push(id);
					if (id[0] == "!") ret.target.room_id.push(id);
					if (id[0] == "@") ret.target.user_id.push(id);
				}
			...
			ret.text.push(event.content.body);
		}

		ret.argv = parse_command(ret.text.join(" "));
		ret.argv[0] = ret.argv[0].slice(1);
		delete ret.text;

		ret.argv = ret.argv.filter((word) => {
			switch (word[0]) {
				case "#":
					ret.target.room_id.push(word);
					return false;
				case "!":
					ret.target.room_id.push(word);
					return false;
				case "@":
					ret.target.user_id.push(word);
					return false;
				default:
					return true;
			}
